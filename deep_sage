...
parser.add_argument('-config', type=str, help = 'The configuration file (contains all the information related to the training/inference session)', required=True)
...
parser.add_argument('-modelDir', type=str, help = 'The pre-trained model directory that is used for inference', required=False)
parser.add_argument('-device', default=0, type=int, help = 'Device to perform requested session on (\'-1\' == cpu and everything else denotes GPU ID)', required=True) 
