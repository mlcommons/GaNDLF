...
# To do
# 1. Now metrics are to be given from yaml
...
def trainingLoop(
    trainingDataFromPickle,
    validationDataFromPickle,
    headers,
    device,
    parameters,
    outputDir,
    ...
    is_regression, is_classification, is_segmentation = find_problem_type(
        headers, model.final_convolution_layer
    )

    if is_regression or is_classification:
        n_classList = len(
            headers["predictionHeaders"]
        )  # ensure the output class list is correctly populated
    ...
    validationDataForTorch = ImagesFromDataFrame(
        ...
        preprocessing=preprocessing,
        in_memory=in_memory,
    )  # may or may not need to add augmentations here
    ...
    train_loader = DataLoader(
        trainingDataForTorch, batch_size=batch_size, shuffle=True, pin_memory=in_memory
    )
    ...
    if n_channels == 0:
        sys.exit(
            "The number of input channels cannot be zero, please check training CSV"
        )
    ...
    if os.path.exists(os.path.join(outputDir, str(which_model) + "_best.pth.tar")):
        checkpoint = torch.load(
            os.path.join(outputDir, str(which_model) + "_best.pth.tar")
        )
        model.load_state_dict(checkpoint["model_state_dict"])
        optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
        print(
            "Model checkpoint found. Loading checkpoint from: ",
            os.path.join(outputDir, str(which_model) + "_best.pth.tar"),
        )
    ...
    scheduler_lr = get_scheduler(
        scheduler, optimizer, batch_size, len(train_loader.dataset), learning_rate
    )
    ...
    log_train.write(
        "Epoch,Train_Loss,Train_Dice,Val_Loss,Val_Dice,Testing_Loss,Testing_Dice\n"
    )
    log_train.close()
    ...
    if use_weights:
        ...
        penaltyData = ImagesFromDataFrame(
            trainingDataFromPickle,
            patch_size,
            headers,
            q_max_length,
        ...
        if not is_regression:
            for batch_idx, (subject) in enumerate(
                ...
                for i in range(0, n_classList):
                    ...
                    dice_weights_dict[i] = (
                        dice_weights_dict[i] + currentNumber
            ...
            dice_weights_dict = {
                k: (v / total_nonZeroVoxels) for k, v in dice_weights_dict.items()
            }  # divide each dice value by total nonzero
            dice_penalty_dict = deepcopy(
                dice_weights_dict
            )  # deep copy so that both values are preserved
            dice_penalty_dict = {
                k: 1 - v for k, v in dice_weights_dict.items()
    ...
    for ep in range(num_epochs):
        ...
        print(
            "\nEp# %03d | LR: %s | Start: %s "
            % (ep, str(optimizer.param_groups[0]["lr"]), str(datetime.datetime.now())),
            flush=True,
        )
        samples_for_train = 0
        model.train()
        for batch_idx, (subject) in enumerate(train_loader):
            ...
            image = torch.cat(
                [subject[key][torchio.DATA] for key in channel_keys], dim=1
            ...
            if is_regression or is_classification:
                ...
                if (
                    model_2d
                ):  # for 2D, add a dimension so that loss can be computed without modifications
            ...
            if not is_regression:
                ...
                if MSE_requested:
                    loss = loss_fn(
                        ...
                        one_hot_mask.double(),
                        n_classList,
                        reduction=loss_function["mse"]["reduction"],
                    )
                else:
                    loss = loss_fn(
                        output.double(),
            ...
            if not is_regression:
                ...
                curr_dice = (
                    MCD(output.double(), one_hot_mask.double(), n_classList)
                    .cpu()
                    .data.item()
                )  # https://discuss.pytorch.org/t/cuda-memory-leakage/33970/3
                # print(curr_dice)
        ...
        if is_segmentation:
            average_train_dice = (
                total_train_dice / samples_for_train
            )  # len(train_loader.dataset)
        else:
            average_train_dice = 1

        average_train_loss = (
            total_train_loss / samples_for_train
        )  # len(train_loader.dataset)
        ...
        average_val_dice, average_val_loss = get_metrics_save_mask(
            model,
            device,
            val_loader,
            patch_size,
            ...
            is_segmentation,
            scaling_factor,
            save_mask=parameters["save_masks"],
            outputDir=os.path.join(outputDir, "validationMasks"),
        )

        # testing data scores
        average_test_dice, average_test_loss = get_metrics_save_mask(
            model,
            device,
            inference_loader,
            patch_size,
            ...
            is_segmentation,
            scaling_factor,
            save_mask=parameters["save_masks"] & testingDataDefined,
            outputDir=os.path.join(outputDir, "testingMasks"),
        )
        ...
        if save_condition_train:
            ...
            torch.save(
                ...
                os.path.join(outputDir, which_model + "_best_train.pth.tar"),
            )

        print(
            "   Train Dice: ",
            format(average_train_dice, ".10f"),
            " | Best Train Dice: ",
            format(best_train_dice, ".10f"),
            " | Avg Train Loss: ",
            format(average_train_loss, ".10f"),
            " | Best Train Ep ",
            format(best_train_idx, ".0f"),
            flush=True,
        )
        ...
        if save_condition_val:
            ...
            torch.save(
                ...
                os.path.join(outputDir, which_model + "_best_val.pth.tar"),
            )

        print(
            "     Val Dice: ",
            format(average_val_dice, ".10f"),
            " | Best Val   Dice: ",
            format(best_val_dice, ".10f"),
            " | Avg Val   Loss: ",
            format(average_val_loss, ".10f"),
            " | Best Val   Ep ",
            format(best_val_idx, ".0f"),
            flush=True,
        )
        ...
        if save_condition_test:
            ...
            torch.save(
                ...
                os.path.join(outputDir, which_model + "_best_test.pth.tar"),
            )

        print(
            "    Test Dice: ",
            format(average_test_dice, ".10f"),
            " | Best Test  Dice: ",
            format(best_test_dice, ".10f"),
            " | Avg Test  Loss: ",
            format(average_test_loss, ".10f"),
            " | Best Test  Ep ",
            format(best_test_idx, ".0f"),
            flush=True,
        )
        ...
        torch.save(
            {
                ...
                "model_state_dict": model.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
                "val_dice": average_val_dice,
                "val_loss": average_val_loss,
            },
            os.path.join(outputDir, which_model + "_latest.pth.tar"),
        )
        ...
        log_train.write(
            ...
            + ","
            + str(average_train_dice)
            + ","
            + str(average_val_loss)
            + ","
            + str(average_val_dice)
            + ","
            + str(average_test_loss)
            + ","
            + str(average_test_dice)
            + "\n"
        )
        log_train.close()
        total_train_dice = 0
...
if __name__ == "__main__":
    ...
    # Write parameters to pickle - this should not change for the different folds,
    # so keeping is independent
    ...
    trainingLoop(
        trainingDataFromPickle=trainingDataFromPickle,
