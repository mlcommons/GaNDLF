...
from GANDLF.logger import Logger, LoggerGAN
from .step import step, step_gan
from .forward_pass import validate_network
from .generic import create_pytorch_objects, create_pytorch_objects_gan
    ...
    for batch_idx, (subject) in enumerate(
        ...
        if params["verbose"]:
            ...
            if ((batch_idx + 1) % (len(train_dataloader) / 2) == 0) and (
                ...
                for metric in params["metrics"]:
                    if isinstance(
                        total_epoch_train_metric[metric], np.ndarray
                    ):
                        to_print = (
...
def training_loop(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if not (first_model_saved) or (
            epoch_valid_loss <= torch.tensor(best_loss)
        ):
