...
def convert_to_nifti(input_dicom_directory, output_file):
    ...
    if not series_IDs:
        raise ValueError("ERROR: given directory \"" + input_dicom_directory + "\" does not contain a valid DICOM series.")
    if len(series_IDs) > 1:
        print("WARNING: given directory \"" + input_dicom_directory + "\" contains more than 1 DICOM series; only the first one, \"" + series_IDs[0] + "\" will be converted.", flush=True)
    series_file_names = sitk.ImageSeriesReader.GetGDCMSeriesFileNames(input_dicom_directory, series_IDs[0])
