...
class RandomColorJitter(RandomTransform, IntensityTransform):
    ...
    def __init__(
        ...
        self.brightness_range = self._parse_range(brightness, 'brightness', min_constraint=0)
        self.contrast_range = self._parse_range(contrast, 'contrast', min_constraint=0)
        self.saturation_range = self._parse_range(saturation, 'saturation', min_constraint=0)
        self.hue_range = self._parse_range(hue, 'hue', min_constraint=-0.5, max_constraint=0.5)
