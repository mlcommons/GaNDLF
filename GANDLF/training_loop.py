...
def trainingLoop(trainingDataFromPickle, validationDataFromPickle, headers, device, parameters, outputDir, holdoutDataFromPickle = None):
    ...
    for ep in range(num_epochs):
        ...
        total_val_dice, total_val_loss = get_stats(model, validationDataForTorch, psize, channel_keys, class_list, loss_fn, dice_penalty_dict)
        average_val_dice = total_val_dice/len(val_loader.dataset)
        average_val_loss = total_val_loss/len(val_loader.dataset)

        # testing data scores
        total_test_dice, total_test_loss = get_stats(model, inferenceDataForTorch, psize, channel_keys, class_list, loss_fn, dice_penalty_dict) 
