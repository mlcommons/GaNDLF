...
def recover_config(modelDir, outputFile):
    assert os.path.exists(modelDir), "The model directory does not appear to exist. Please check parameters."

    pickle_location = os.path.join(modelDir, "parameters.pkl")
    assert os.path.exists(pickle_location), "The model does not appear to have a configuration file. Please check parameters."
