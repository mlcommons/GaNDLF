...
def preprocess_and_save(data_csv, config_file, output_dir, label_pad_mode="constant"):
    ...
    for _, (subject) in enumerate(
        ...
        print("===Debug===")
        print("Origin:", image_for_info_copy.GetOrigin(), flush=True)
        print("Spacing:", image_for_info_copy.GetSpacing(), flush=True)
        print("Spacing_correct:", correct_spacing_for_info_copy, flush=True)
        for channel in parameters["headers"]["channelHeaders"]:
            image_file = Path(
                os.path.join(
                    current_output_dir,
