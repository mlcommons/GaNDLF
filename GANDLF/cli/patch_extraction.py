...
from GANDLF.data.patch_miner.opm.dsa_annotations import extract_patches
...
def patch_extraction(
    ...
    for sid, slide, label in parse_gandlf_csv(input_path):
        if label.lower().endswith(".json"):
            use_dsa(sid,slide,label, cfg, output_path)
        else:
            use_opm(slide, label,sid,cfg, output_path, original_patch_size)


def use_dsa(sid,slide,label,cfg,output_path):
    out_csv_path = os.path.join(output_path, "dsa_train.csv")
    extract_patches(label, slide,cfg["patch_size"][0], output_path,sid)


def use_opm(slide, label,sid,cfg, output_path, original_patch_size):
