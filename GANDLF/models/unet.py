...
class unet(ModelBase):
    ...
    def __init__(self, n_dimensions, n_channels, n_classes, base_filters, final_convolution_layer):
        ...
        self.de_3 = DecodingModule(base_filters*16, base_filters*8, self.Conv, self.Dropout, self.InstanceNorm)
        self.us_2 = UpsamplingModule(base_filters*8, base_filters*4, self.Conv, self.Dropout, self.InstanceNorm)
        self.de_2 = DecodingModule(base_filters*8, base_filters*4, self.Conv, self.Dropout, self.InstanceNorm)
        self.us_1 = UpsamplingModule(base_filters*4, base_filters*2, self.Conv, self.Dropout, self.InstanceNorm)
        self.de_1 = DecodingModule(base_filters*4, base_filters*2, self.Conv, self.Dropout, self.InstanceNorm)
        self.us_0 = UpsamplingModule(base_filters*2, base_filters, self.Conv, self.Dropout, self.InstanceNorm)
        self.out = out_conv(base_filters*2, n_classes, self.Conv, self.Dropout, self.InstanceNorm, final_convolution_layer = self.final_convolution_layer)
