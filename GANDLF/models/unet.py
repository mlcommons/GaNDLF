...
class unet(ModelBase):
    ...
    def __init__(
        ...
        if patch_check != parameters["model"]["depth"] and patch_check >= 4:
            print(
                "The patch size is not large enough for desired depth. It is expected that each dimension of the patch size is divisible by 2^i, where i is in a integer greater than or equal to 4. Only the first %d layers will run."
                % patch_check
            )
        elif patch_check != parameters["model"]["depth"] and patch_check < 4:
            sys.exit(
                "The patch size is not large enough for desired depth. It is expected that each dimension of the patch size is divisible by 2^i, where i is in a integer greater than or equal to 4."
            )
