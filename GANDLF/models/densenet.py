...
class _DenseLayer(nn.Sequential):
    ...
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, BatchNorm, Conv):
        super().__init__()
...
class _DenseBlock(nn.Sequential):
    ...
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate,
                 drop_rate, batch_norm, conv):
        super().__init__()
        for i in range(num_layers):
            layer = _DenseLayer(num_input_features + i * growth_rate,
                                growth_rate, bn_size, drop_rate, batch_norm, conv)
            self.add_module('denselayer{}'.format(i + 1), layer)


class _Transition(nn.Sequential):

    def __init__(self, num_input_features, num_output_features, BatchNorm, Conv, AvgPool):
        super().__init__()
...
class DenseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.adaptive_avg_pool(out,
                                    output_size=self.output_size).view(features.size(0), -1)
