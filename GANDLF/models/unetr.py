...
class _MSA(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_weights = self.softmax(torch.matmul(query, key.transpose(-1, -2)) / math.sqrt(self.query_size))
        self_attention = torch.matmul(attention_weights, value)
        self_attention = self_attention.permute(0, 2, 1, 3).contiguous()
        self_attention = self_attention.view(list(self_attention.size()[:-2]) + [self.all_heads]) # reshape
...
class _Transformer(nn.Sequential):
    def __init__(self, img_size, patch_size, in_feats, embed_size, num_heads, mlp_dim, num_layers, out_layers, Conv, Norm):
        super().__init__()
        ...
        for _ in range(0, num_layers):
            layer = _TransformerLayer(img_size, embed_size, num_heads, mlp_dim, Conv, Norm)
...
class unetr(ModelBase):
    ...
    def __init__(
        ...
        if not ("num_heads" in parameters["model"]):
            ...
            print("Default number of heads in multi-head self-attention (MSA) set to 12.")

        if not ("embed_dim" in parameters["model"]):
            parameters["model"]["embed_dim"] = 768
            print("Default size of embedded dimension set to 768.")

        patch_check = checkImgSize(parameters["patch_size"], number=parameters["model"]["depth"])
        ...
        for i in range(0, self.depth - 1):
            ...
            tempconvs.add_module("conv0", _DeconvConvBlock(self.embed_size, 32*2**self.depth, self.Norm, self.Conv, self.ConvTranspose))

            for j in range(self.depth - 2, i, -1):
                tempconvs.add_module("conv%d"%j, _DeconvConvBlock(128*2**j, 128*2**(j-1), self.Norm, self.Conv, self.ConvTranspose))

            self.convs.append(tempconvs)

            # add upsampling
            self.upsampling.append(_UpsampleBlock(128*2**(i+1), self.Norm, self.Conv, self.ConvTranspose))

        # add upsampling for transformer output (no convs)
        self.upsampling.append(self.ConvTranspose(
        ...
        self.input_conv.add_module("conv1", _ConvBlock(self.n_channels, 32, self.Norm, self.Conv))
        ...
        self.output_conv.add_module("conv3", out_conv(64, self.n_classes, 
                conv_kwargs = {"kernel_size": 1, "stride": 1, "padding": 0, "bias": False}, 
                norm = self.Norm, 
                conv = self.Conv,
    ...
    def forward(self, x):
        ...
        y = self.upsampling[-1](transformer_out[-1].transpose(-1, -2).view(-1, self.embed_size, *self.patch_dim)) # z12

        for i in range(len(self.convs)-1,-1,-1):
            zi = transformer_out[i].transpose(-1, -2).view(-1, self.embed_size, *self.patch_dim)
            zi = self.convs[i](zi)
