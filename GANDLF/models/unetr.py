...
class unetr(ModelBase):
    ...
    def __init__(
        ...
        for i in range(0, self.depth - 1):
            ...
            tempconvs.add_module("conv0", _DeconvConvBlock(self.embed_size, 32*2**self.depth, self.Norm, self.Conv))

            for j in range(self.depth - 2, i, -1):
                tempconvs.add_module("conv%d"%j, _DeconvConvBlock(128*2**j, 128*2**(j-1), self.Norm, self.Conv))

            self.convs.append(tempconvs)

            # add upsampling
            self.upsampling.append(_UpsampleBlock(128*2**(i+1), self.Norm, self.Conv))

        # add upsampling for transformer output (no convs)
        self.upsampling.append(nn.ConvTranspose3d(
