...
def validate_network_ov(
    ...
    for batch_idx, (subject) in enumerate(
        ...
        if 'single_run' in params.keys() and params['single_run']:
            if batch_idx > 0:
                break

        if is_recording:
            if torch.rand(1) < params['model']['save_data_subsample']:
                continue
        ...
        if ("value_keys" in params) and label_present:
            ...
            for patches_batch in patch_loader:
                ...
                result = step_ov(model, image, label, params)
                ...
                if params["medcam_enabled"]:
                    ...
                    if is_recording:
                        # print("Batch idx is: ", batch_idx, current_patch)
                        image_patch_data.append(image.cpu().detach().numpy())
                        pred_patch_data.append(output.cpu().detach().numpy())
                        label_patch_data.append(label.cpu().detach().numpy())
    ...
    if is_recording:
        if "save_data_filename" in params["model"].keys():
            np.savez(params['model']['save_data_filename'], data=image_patch_data, pred=pred_patch_data, label=label_patch_data)
        else:
            np.savez("quantization.npz", data=image_patch_data, pred=pred_patch_data, label=label_patch_data)
