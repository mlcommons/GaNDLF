...
for i in range(5):
	checkpoint = torch.load(os.path.join(args.torch_model_dir, str(i), args.model_name+'_best.pth.tar'), map_location=torch.device('cpu'))
	...
	torch.onnx.export(model,
                dummy_input.to("cpu"),
                os.path.join(args.onnx_model_dir, str(i), args.model_name+'_best.onnx'),
                opset_version=11,
                export_params=True,
                verbose=True,
                input_names = ['input'],
