...
def main_worker(current_gpu, config: SampleConfig):
    ...
    if is_export_only:
        ...
        nncf_config = register_default_init_args(
            ...
            validate_fn=lambda *x: validate_model_fn(*x)[::0],
    ...
    if 'train' in config.mode:
        if is_accuracy_aware_training(config):
            ...
            #train(config, compression_ctrl, model, lr_scheduler, model_name, optimizer,
...
def create_datasets(config):
    data_dir = osp.join(config['root_dir'], config['nncf_data_dir'], str(config['N_FOLD']))
    input_data=osp.join(data_dir, 'train', config['nncf_data_name'])
    npzfiles = np.load(input_data, allow_pickle = True )
    image, pred_label, gt_label = npzfiles.files
    train_images = npzfiles[image]
    length = train_images.shape[0]
    train_images = np.squeeze(train_images)
    train_pred_label = npzfiles[pred_label]
    train_pred_label = np.squeeze(train_pred_label, axis=3)
    train_pred_label = np.argmax(train_pred_label, axis=2)
    train_gt_label = npzfiles[gt_label]

    input_data=osp.join(data_dir, 'validation', config['nncf_data_name'])
    npzfiles = np.load(input_data, allow_pickle = True )
    image, pred_label, gt_label = npzfiles.files
    val_images = npzfiles[image]
    length = val_images.shape[0]
    val_images = np.squeeze(val_images)
    val_pred_label = npzfiles[pred_label]
    val_pred_label = np.squeeze(val_pred_label, axis=3)
    val_pred_label = np.argmax(val_pred_label, axis=2)
    val_gt_label = npzfiles[gt_label]

    train_tensor_images = torch.Tensor(train_images) # transform to torch tensor
    train_tensor_labels = torch.Tensor(train_gt_label)

    val_tensor_images = torch.Tensor(val_images) # transform to torch tensor
    val_tensor_labels = torch.Tensor(val_gt_label)

    train_dataset = torch.utils.data.TensorDataset(train_tensor_images, train_tensor_labels) # create your datset
    val_dataset = torch.utils.data.TensorDataset(val_tensor_images, val_tensor_labels) # create your datset
...
def train_epoch(train_loader, model, criterion, criterion_fn, optimizer, compression_ctrl, epoch, config,
    ...
    for i, (input_, target) in enumerate(train_loader):
        ...
        target = target.to(config.device).squeeze().type(torch.LongTensor)
