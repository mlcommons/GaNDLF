...
if mode != 0: # training mode
    ...
    InferenceManager(dataframe = data_full, augmentations = augmentations, psize = psize, channelHeaders = channelHeaders, labelHeader = labelHeader, model_parameters_file = model_parameters, outputDir = model_path, batch_size = batch_size, which_loss = which_loss, n_classes = n_classes, base_filters = base_filters, n_channels = n_channels, which_model = which_model, parallel_compute_command = parallel_compute_command, device = args.device)
